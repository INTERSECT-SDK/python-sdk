...
Note that there are some aspects of intersect_sdk_version_resolver which CANNOT be tested,
...
from intersect_sdk._internal.version_resolver import (
...
def test_intersect_sdk_version_info() -> None:
    assert len(intersect_sdk_version_info) == 3
    assert all(isinstance(x, int) for x in intersect_sdk_version_info)
...
def test_equal_intersect_sdk_version_ok() -> None:
    assert (
        resolve_user_version(intersect_sdk_version_string, 'source', IntersectDataHandler.MESSAGE)
        is True
    )


def test_bugfix_up_ok() -> None:
    mock_version = f'{intersect_sdk_version_info[0]}.{intersect_sdk_version_info[1]}.{intersect_sdk_version_info[2] + 1}'
...
def test_major_difference_up_not_ok(caplog: pytest.LogCaptureFixture) -> None:
    mock_version = f'{intersect_sdk_version_info[0] + 1}.{intersect_sdk_version_info[1]}.{intersect_sdk_version_info[2]}'
...
def test_minor_intersect_sdk_version_up_ok_if_release() -> None:
    their_version = '1.1.0'
    our_version = '1.0.0'
    assert (
        _resolve_user_version(
...
def test_minor_intersect_sdk_version_down_ok_if_release() -> None:
    their_version = '1.1.0'
    our_version = '1.2.0'
    assert (
        _resolve_user_version(
...
def test_minor_intersect_sdk_version_up_not_ok_if_prerelease(
...
def test_minor_intersect_sdk_version_down_not_ok_if_prerelease(
    caplog: pytest.LogCaptureFixture,
) -> None:
    their_version = '0.2.0'
    our_version = '0.3.0'
