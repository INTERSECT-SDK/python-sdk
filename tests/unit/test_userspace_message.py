...
def test_valid_userspace_message_deserializes():
    serialized = b'{"messageId":"cc88a2c9-7e47-409f-82c5-ef49914ae140","operationId":"operation","contentType":"application/json","payload":"payload","headers":{"source":"source","destination":"destination","service_version":"0.0.1","sdk_version":"0.5.0","created_at":"2024-01-19T20:21:14.045591Z","data_handler":0}}'
...
def test_unusual_userspace_message_deserializes():
    serialized = b'{"messageId":"cc88a2c9-7e47-409f-82c5-ef49914ae140","operationId":"operation","contentType":"application/json","payload":"payload","headers":{"source":"source.one","destination":"destination.two","service_version":"0.0.1","sdk_version":"0.5.0","created_at":"2024","data_handler":0}}'
    deserialized = deserialize_and_validate_userspace_message(serialized)
    assert deserialized['headers']['data_handler'] == IntersectDataHandler.MESSAGE
    assert deserialized['contentType'] == IntersectMimeType.JSON
    assert deserialized['headers']['has_error'] is False
...
def test_missing_headers_properties_does_not_deserialize():
    ...
    assert ('headers', 'service_version') in locations
    assert ('headers', 'sdk_version') in locations


def test_invalid_does_not_deserialize():
    serialized = b'{"messageId":"notauuid","operationId":1,"contentType":"doesnotexist","payload":"payload","headers":{"source":"/","destination":"/","service_version":"1.0.0-alpha","sdk_version":"1.0.0+20130313144700","created_at":"2024-01-19T20:21:14.045591","data_handler":-1}}'
    with pytest.raises(ValidationError) as err:
        deserialize_and_validate_userspace_message(serialized)
    ...
    assert {'type': 'string_pattern_mismatch', 'loc': ('headers', 'service_version')} in errors
...
def test_create_userspace_message():
    ...
    assert msg['headers']['service_version'] == '1.0.0'
